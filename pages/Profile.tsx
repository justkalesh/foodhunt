...
const Profile: React.FC = () => {
   ...
   const handleSave = async () => {
      ...
      if (res.success && res.data) {
         ...
         setDisplayUser(res.data); // Update local state for view
         setIsEditing(false);
      }
      setSaving(false);
   };
   ...
   return (
      <div className="max-w-4xl mx-auto px-4 py-8">
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                     ...
                     <div className="flex flex-col items-center md:items-end gap-2">
                        ...
                        <div className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                           {displayUser.loyalty_points || 0} Points
                        </div>
                     </div>
                  </div>
         ...
         <div className={`grid gap-8 ${isOwnProfile ? 'md:grid-cols-3' : 'grid-cols-1'}`}>
            {/* Left Column: Settings & Theme (Only for own profile) */}
            {isOwnProfile && (
                     ...
                     {activity.recentReviews.length > 0 ? activity.recentReviews.map(r => (
                        <div key={r.id} className="pb-3 border-b border-gray-100 dark:border-gray-700 last:border-0 last:pb-0">
                           <div className="flex justify-between text-sm mb-1">
                              <span className="font-semibold dark:text-white">{(r as any).vendor_name || r.vendor_id}</span>
